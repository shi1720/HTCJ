...
Secrets belong in the deployment secret store or a local untracked `.env`. API users should not send secrets through source URL query strings. Backups need access controls and retention policies appropriate to the pilot. Single-use recovery keys provide self-service recovery without an email provider. Their 256-bit secret is shown only when created and stored only as a SHA-256 digest. Recovery consumes the old key, sets a new password, replaces the key and revokes all previous sessions in one transaction. Password changes require the current password and revoke all prior sessions; they retain the saved recovery key. Credential mutation rechecks the original credential version after asynchronous password hashing, so concurrent resets cannot leave an old-password session active. Existing accounts can generate a key after password reauthentication. Recovery and reauthentication have separate durable per-identity throttles; generic recovery failures avoid disclosing whether an account exists. A maximum of 20 sessions per account bounds storage.

Recovery is based on possession of the saved key, not verification of email ownership or an identity document. If both password and key are lost, this release has no automated recovery. Protect exported keys like passwords; rotate a key if its confidentiality is uncertain. There is no email delivery or administrator identity-verification workflow.

A broader deployment should add organization identity lifecycle controls, service-health monitoring, backup restoration drills, and an external review before expanded access.
